---
layout: page
title: User Documentation
---

# User Documentation

These guides are written for busy real estate agents. No technical background needed.

## Start Here

1. [Getting Started](getting-started.html)
2. [Create and Manage Campaigns](campaigns/create-campaigns.html)
3. [Bulk Upload Leads (CSV)](campaigns/bulk-upload.html)
4. [Approve and Send Workflow](campaigns/approve-and-send.html)
5. [Dashboard and Metrics](dashboard-and-metrics.html)
6. [Billing and Plans](billing-and-plans.html)
7. [Troubleshooting](troubleshooting.html)

## What Dripr Helps You Do

Dripr sends monthly real estate update emails for your contacts so you stay top-of-mind.

- **Address Newsletter**: Use when you have the contact's property address.
- **No-Address Newsletter**: Use when you do not have an address, but you do have ZIP code.

If you are brand new, start with the Getting Started guide and create 1-2 test leads first.
